...
def test_validate_zipfile__valid(testdata):
    assert is_zipfile(testdata.abspath('data/zip/sample.zip')) # based on its magic number

def test_validate_zipfile__invalid(testdata):
    assert not is_zipfile(testdata.abspath('data/zip/sample.txt.zip'))

def test_get_filelist_without_extraction(testdata):
...
@pytest.mark.xfail
def test_check_integrity(testdata):
    zipfile_broken = testdata.abspath('data/zip/sample_broken.zip')
