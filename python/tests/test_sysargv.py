...
def test_argv__expansions_performed_and_quotation_removed(workspace):
    ...
    python run\ it.py --var1 "$VAR1" --var2="$VAR2" --var3="three" -f ~/.bashrc *
    ''')
    assert r.out == \
        "['run it.py', '--var1', '$HOME in single quotes', " \
        "'--var2=/root in double quotes', '--var3=three', '-f', " \
        "'/root/.bashrc', " \
